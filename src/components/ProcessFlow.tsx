                ...
                <div className="relative">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 relative z-10">
                        {steps.map((step, index) => (
                            ...
                            >
                                ...
                                <div className="relative mb-6">
                                    <div className="h-20 w-20 rounded-full bg-industrial-blue border-4 border-gray-600 flex items-center justify-center group-hover:border-safety-yellow group-hover:scale-110 transition-all duration-300 shadow-xl">
                                        <div className="text-safety-yellow group-hover:text-white transition-colors duration-300 transform group-hover:rotate-12">
                                            {step.icon}
                                        </div>
                                    </div>
                                    {/* Number Badge */}
                                    <div className="absolute -top-2 -right-2 h-8 w-8 rounded-full bg-safety-yellow flex items-center justify-center text-xs font-bold text-black border-2 border-industrial-blue">
                                        0{step.id}
                                    </div>
                                </div>

                                {/* Text Content */}
                                <h3 className="text-xl font-bold uppercase text-white font-header mb-2 group-hover:text-safety-yellow transition-colors">{step.title}</h3>
                        ...
                        >
                            {/* Image Bubble */}
                            <div className="relative mb-6">
                                <div className="h-40 w-40 rounded-2xl bg-safety-yellow border-4 border-white flex items-center justify-center group-hover:scale-105 transition-all duration-300 shadow-xl overflow-hidden">
                                    <img
                                ...
                                {/* Success Badge */}
                                <div className="absolute -bottom-3 -right-3 h-10 w-10 rounded-full bg-green-500 flex items-center justify-center text-white border-4 border-dark-slate shadow-lg z-10">
                                    <Flag className="h-5 w-5" />
                                </div>
                            </div>

                            {/* Text Content */}
                            <h3 className="text-xl font-bold uppercase text-safety-yellow font-header mb-2">The Result</h3>
                            <p className="text-white text-sm max-w-[200px] font-medium italic">"A Confident, Satisfied Client."</p>
                        </motion.div>
                    </div>
                </div>
