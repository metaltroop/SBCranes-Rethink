...
import towerCraneImg from '../assets/images/buildingcrae.png';
                ...
                <div className="flex flex-col gap-6 md:hidden">
                    {projects.map((project, index) => (
                        <Link
                            to="#"
                            key={project.id}
                            className="group flex items-stretch bg-gray-800 border border-gray-700 rounded-lg overflow-hidden shadow-lg hover:border-safety-yellow transition-all duration-300"
                        >
                            ...
                            <div className={`p-4 flex flex-col justify-center flex-1 ${index % 2 === 1 ? 'order-1 text-right' : 'order-2 text-left'}`}>
                                <span className="text-[10px] font-bold uppercase tracking-widest text-safety-yellow mb-1">{project.category}</span>
                                <h3 className="text-lg font-bold uppercase text-white font-header leading-tight mb-2 group-hover:text-safety-yellow transition-colors">{project.title}</h3>
                                <div className="h-0.5 w-8 bg-gray-600 mb-2 group-hover:bg-safety-yellow transition-colors inline-block"></div>
                            </div>
                        </Link>
                    ))}
                </div>
                    ...
                    {projects.map((project) => (
                        ...
                        >
                            ...
                            <div className="absolute bottom-0 left-0 p-6">
                                <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-safety-yellow">{project.category}</span>
